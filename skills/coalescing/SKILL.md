...
runbook amendments, harvest and retire completed plans, promote recurring
workflows to skills, and propose cross-repo fold-ups upward when a rule
generalizes beyond this repository. Keeps the always-read documentation
tier small, accurate, and hot while git history holds everything raw.
...
(This repo's lessons ledger is flat and chronological — a repo-wide dated
H2 count with a date watermark; see `docs/coalescing.md`.)
...
`local-only pin`; `bin/coalesce-check` reports
...
6. Decay evidence is multi-signal: absence of citation alone never
   ...
   **expired** and folds to git with the version fact as its cue, no
   ...
   never a "our conventions" home. Golden rules and safety invariants are
   exempt from automated decay entirely (importance floor); they change
   only by
...
(hub [AT-THEORY-7], "What ascends here" — agent-theory repository).
**In the guidance repo**,
...
- `bin/coalesce-check` is a provenance trail (process sense, not
  [EVC-*] product evidence), not a second recipe: the
