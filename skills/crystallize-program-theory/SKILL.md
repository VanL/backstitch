...
Status: Active — adopted from agent-theory @ `ec716e8` via
`docs/plans/2026-08-08-agent-theory-delta-wave-propagation-plan.md`.
This repository's product theory was crystallized with this skill
(hub-side) on 2026-08-08; the copy is here for module theory and future
revisions. Terminology: the ALT/REV template field `Evidence:` below is
the hub's shared record grammar and uses the provenance sense (see the
theory file's terminology note), not this repository's [EVC-*] product
sense.
...
theory would become too long for general entry. See the hub's program
theory [AT-THEORY-2.1] (agent-theory repository).
...
- Editing the **hub's** theory of agent-theory itself (that file lives
  in the agent-theory repository and is edited there under its normal
  review).
...
- Definitional primer: `docs/specs/09-agent-theory-and-program-theory.md`
  (this repository's slot for the hub's `02-` primer)
- Hub meta-theory: the agent-theory repository's program theory,
  [AT-THEORY-0]–[AT-THEORY-8] (read there; this repository's
  `docs/program-theory.md` is the backstitch product account)
...
1. Existing `docs/program-theory.md` (the current account — here:
   backstitch product theory, Active).
...
- This skill is the sole home for the interview spine, ALT/REV field templates,
  ...
  reference). Do not re-expand those into the hub's program theory
  (agent-theory repository) or this repository's product theory.
