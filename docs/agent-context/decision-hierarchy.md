...
- When the plan includes a `## Proposed Spec Delta`, record both the **spec
  ...
  **promotion baseline identifier** (after the delta is applied to
  `docs/specs/`). Use a commit SHA when committed; otherwise diff base plus
  worktree state. Mid-implementation claims are against the promotion
  baseline — not plan appendix text and not the pre-promotion identifier.
...
- **Spec-changing implementation** (see `runbooks/writing-plans.md` §4b–4d):
  the plan carries exact proposed sections for review; the **spec-promotion
  slice** (early, not last) applies them per a named strategy (A/B/C/D).
  v1 glob rungs are **per-file** only. Do not implement code that cites
  `spec_roots` paths against plan-only text while `docs/specs/` still reflects
  the baseline. **Exploration** (behavior undecided) is not implementation
  against a governing spec — when behavior is decided, promote to `spec_roots`
  first, then build. Prose `Status: Proposed` and glob rungs are different
  mechanisms — see §4d.
- **Single governing contract:** after promotion, `docs/specs/` is canonical.
  The plan's delta is historical review material, not a parallel source of
  truth.
...
closed chain, and any project self-corpus / traceability command named in the
plan has been rerun from the current state. For backstitch implementation work,
`backstitch check --repo-root .` with zero errors and zero warnings is
**mandatory** (`AGENTS.md`, [SC-10]) — not waivable. Clearing reciprocal
backlink debt and completing glob graduation (when strategy C was used) are part
