...
- the governing spec (baseline SHA) and, when present, the plan's
  `## Proposed Spec Delta`
- the active plan
...
- **Principle-Level Diagnosis:** when findings are not independent
  ...
  produced it; the principle dictates the remediation's shape (a
  violation of engineering principle §12 — Enumerable Contracts Get
  Executable Gates, this repo's numbering — gets a firing gate, not
  another prose rule).
