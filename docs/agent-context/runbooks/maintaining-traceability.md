...
- if intended behavior changed, update the spec in the same change — for
  ...
  cites new `spec_roots` paths, unless the plan is typed exploration (no
  governing spec yet)
...
- closure review diffs **every planned task** against an executable
  verification result: a checked box, a passing default suite, or a
  test that exists but is gated off is not verification — a
  skipped-everything run exits 0
...
- for spec-changing work: promotion baseline identifier recorded; promotion
  strategy executed; glob graduation (if any) completed with citation updates;
  self-corpus / traceability gate rerun (named command and result — for
  backstitch: zero errors and zero warnings, mandatory)
