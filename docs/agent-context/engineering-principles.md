...
A canonical converter means the next agent doesn't re-derive the format. A
blast-radius note means the next change knows its impact zone. A failing test
means the next debugging session starts from known-good. A lesson written down
means the next session doesn't repeat the mistake.
