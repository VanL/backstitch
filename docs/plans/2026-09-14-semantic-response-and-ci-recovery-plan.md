...
| 3 | Align analyzer generation with required evidence | One nested `assessment` schema implemented; full hermetic tests, benchmark tests, static gates, self-corpus `check`, and live OpenAI qualification for `gpt-5.6-luna` and `gpt-5.5-2026-04-23` passed. Hosted run `34984575399` then showed Ollama duplicating coordinates in both responses. | A flat mixed-role array would need a more complex conditional/containment vocabulary. Role-keyed arrays let `anyOf`, `required`, and `minItems` express the existing role matrix directly. The observed Ollama failure justifies `uniqueItems` on role arrays; local duplicate rejection remains authoritative. Requalification is pending. |
